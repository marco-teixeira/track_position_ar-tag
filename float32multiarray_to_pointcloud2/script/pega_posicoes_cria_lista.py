...
def callbacOdom(data):
        global robotPose
        robotPose[0] = data.pose.pose.position.x;
        robotPose[1] = data.pose.pose.position.y;
	robotPose[2] = data.pose.pose.position.z;
...
def publicaPosicao(data):
        ...
        move_goal.publish(data)

def limpaMapa(data):
        pub_pose = PoseWithCovarianceStamped()
        try:
                (trans,rot) = listener.lookupTransform('/map', str(frame), rospy.Time(0))
                robotPose = trans
...
if __name__ == '__main__':
	...
	rospy.Subscriber("/odom", Odometry, callbacOdom)
        #publisher
        move_goal = rospy.Publisher("/ponto_desejado", PointStamped)
